...
class EduKidsApp {
    ...
    setupEventListeners() {
        ...
        // Configura event listeners para o sistema de dicas animado
...
document.getElementById('next-question-btn').disabled = false;
    ...
    showHint() {
        console.log('🔍 showHint chamada!');
        const subject = this.subjects[this.currentSubject];
        const question = subject.questions[this.currentQuestionIndex];
        
        console.log('🔍 Pergunta atual:', question);
        console.log('🔍 Dica da pergunta:', question.hint);
        
        // Usa o novo sistema de dicas moderno
        this.showHintMessage(question.hint);
    ...
    showHintMessage(message) {
        console.log('🔍 showHintMessage chamada com:', message);
        const hintContainer = document.getElementById('hint-container');
        const hintText = document.getElementById('hint-text');
        
        console.log('🔍 hintContainer encontrado:', hintContainer);
        console.log('🔍 hintText encontrado:', hintText);
        
        if (!hintContainer) {
            console.error('❌ hint-container não encontrado!');
            return;
        }
        
        if (!hintText) {
            console.error('❌ hint-text não encontrado!');
            return;
        }
        
        // Define a mensagem da dica
        hintText.textContent = message;
        
        // Mostra o leão com animação
        hintContainer.classList.remove('hidden');
        hintContainer.classList.add('show');
        
        console.log('✅ Dica mostrada com sucesso!');
    ...
    hideHintMessage() {
        ...
        // Remove o hidden após a animação
        setTimeout(() => {
    ...
    // Função para demonstrar o sistema de dicas (opcional)
    demonstrateHintSystem() {
        const messages = [
            "💡 Olá! Eu sou seu assistente de dicas!",
            "🎯 Quando você clicar em 'Dica', eu apareço aqui do lado!",
            "✨ Veja como eu flutuo suavemente para chamar sua atenção!",
            "🔔 Você pode fechar clicando no X ou pressionando ESC!",
            "🌟 Eu me adapto ao modo escuro automaticamente!",
            "🎓 Sempre que precisar de ajuda, eu estarei aqui!"
        ];
        ...
        const showNextMessage = () => {
            if (messageIndex < messages.length) {
                this.showHintMessage(messages[messageIndex]);
                messageIndex++;
                setTimeout(showNextMessage, 8000); // 8 segundos entre mensagens
            }
        };

        // Inicia a demonstração após 3 segundos
        setTimeout(showNextMessage, 3000);
    }

    // Função para mostrar dica especial
    showSpecialHint(message) {
